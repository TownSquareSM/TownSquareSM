...
<div class="comments-item" id="comments-item-<?php echo $comment->id; ?>">
	<div class="row">
		<div class="col-md-1">
        	<?php if(!$params['businesspage']){ ?>
            		<img class="comment-user-img" src="<?php echo $user->iconURL()->smaller; ?>" />
            <?php } else { ?>
            		<img class="comment-user-img" src="<?php echo $params['businesspage']->photoURL('smaller'); ?>" />				
			<?php }?>            
		</div>
		<div class="col-md-11">
				...
				<p>
					<?php
						if(!$params['businesspage']){
			 				...
			 				echo ossn_plugin_view('output/url', array(
									'href' => ossn_site_url("page/view/{$params['businesspage']->guid}"), 
									'text' => $params['businesspage']->title, 
									'class' => 'owner-link',
			 	 			));						
						}
